# 👋 Hi, I'm Aina!
### AI & Machine Learning Engineer | UiTM Intelligent Systems Graduate

Welcome to my technical portfolio. I specialize in building neuro-symbolic AI systems, RAG pipelines, and secure enterprise tools.

---

## 📂 Featured Projects

### 1. 🤖 Expert Corporate Intelligence Agent (RAG)
**Tech Stack:** Python, LangChain, Pinecone, Gemini 2.5 Pro
* **What it is:** An enterprise-grade system that allows employees to query internal financial documents with "Zero-Hallucination" accuracy.
* **Key Tech:** Vision-First OCR, Hybrid Search (Dense + Sparse), Cross-Encoder Reranking.
* [🔗 View Project Architecture & Case Study](/RAG-Portfolio-Showcase/) <--- *Link to your other repo's GitHub Page*

### 2. 📊 Another Data Project (Example)
**Tech Stack:** Pandas, Scikit-Learn, Streamlit
* **What it is:** A predictive analytics dashboard for sales forecasting.
* [🔗 View Project Code](https://github.com/404aina/project-2)

---

## 🛠️ Technical Skills
* **Languages:** Python (Advanced), SQL, JavaScript
* **AI/ML:** TensorFlow, PyTorch, LangChain, Google Vertex AI
* **Cloud & DevOps:** Docker, Google Cloud Platform (GCP), Firebase

---

## 📬 Contact Me
* [LinkedIn](https://linkedin.com/in/yourprofile)
* [Email](mailto:your.email@example.com)
